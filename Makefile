APP_NAME := NewNet
APP_PROJECT := NewNet.xcodeproj
APP_WORKSPACE := NewNet.xcworkspace
APP_SCHEME ?= NewNet
APP_PLATFORM := macos
...
PHONY_TARGETS := $(TARGET_PREFIX)help $(TARGET_PREFIX)diagnose $(TARGET_PREFIX)build \
	...
	$(TARGET_PREFIX)build-and-run-background $(TARGET_PREFIX)release $(TARGET_PREFIX)package \
	$(TARGET_PREFIX)xcode-open $(TARGET_PREFIX)clean $(TARGET_PREFIX)agent-verify
.PHONY: $(PHONY_TARGETS)
...
$(TARGET_PREFIX)help:
	@printf "%s\n" \
		...
		"  make $(TARGET_PREFIX)release                  Build Release configuration" \
		"  make $(TARGET_PREFIX)package                  Build release DMG via makedmg.sh" \
		"  make $(TARGET_PREFIX)xcode-open               Open the project in Xcode" \
...
$(TARGET_PREFIX)release:
	@$(MAKE) --no-print-directory $(TARGET_PREFIX)build CONFIGURATION=Release

$(TARGET_PREFIX)package:
	@./makedmg.sh

$(TARGET_PREFIX)xcode-open:
	@open "$(PROJECT)"
